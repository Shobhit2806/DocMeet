...
const {
  Appointment,
  validateAppointment,
} = require("../models/appointmenttime");
const app = express();
...
router.get("/:DoctorEmailId", auth, async (req, res) => {
  ...
  const appointments = await Appointment.find({
    DoctorEmailId: req.params.DoctorEmailId,
  }).sort({ date: 1, time: 1 });
  res.send(appointments);
});
...
router.get("/patient/:PatientEmailId", async (req, res) => {
  const appointments = await Appointment.find({
    PatientEmailId: req.params.PatientEmailId,
  }).sort({ date: 1, time: 1 });
  res.send(appointments);
});
...
router.post("/", async (req, res) => {
  ...
  let appointment = new Appointment({
    ...
    PatientEmailId: req.body.PatientEmailId,
    date: req.body.date,
    time: req.body.time,
